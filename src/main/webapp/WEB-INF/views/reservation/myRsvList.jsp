  ...
  <div class="tab-pane fade ${roomShowActive }" id="tab-room" role="tabpanel" aria-labelledby="room-tab">
	<p>
  	...
  	<form id="roomSearchForm" method="post">
  		...
  		<input type="hidden" name="tab" id="tab" value="room"/>
    	...
    	<div class="d-flex align-items-center justify-content-between py-3 border-300 px-lg-6 px-4 notification-card border-top read">
	         <div class="d-flex">
	           <div class="me-3 flex-1 mt-2">
	             ...
	             <p class="fs-1"><span class="fw-bold">회의실 이름 : </span>${room.roomName }</p>
	         ...
	         <div class="font-sans-serif">
				...
				<c:set var="ttime">
	          	...
	          	<c:if test="${(roomDate eq tdate and room.rrStarttime > ttime) or (roomDate eq tdate and room.rrEndtime > ttime)}">
		           <button class="btn btn-phoenix-danger btn-sm" onclick="location.href='/myroomcancle.do?rrCd=${room.rrCd}'" name="cancleBtn" data-cd="${room.rrCd }">
    	...
    	<div class="d-flex align-items-center justify-content-between py-3 border-300 px-lg-6 px-4 notification-card border-top read">
	         ...
	         <div class="font-sans-serif">
           	   <c:if test="${(assetDate eq tdate and asset.asrStarttime > ttime) or (assetDate eq tdate and asset.asrEndtime > ttime)}">
		           <button class="btn btn-phoenix-danger btn-sm" onclick="location.href='/myassetcancle.do?asrCd=${asset.asrCd}'" data-cd="${asset.asrCd }">
				      <span class="text-danger" data-feather="x-circle"></span> 취소
				   </button>
	           	</c:if>
	         </div>
...
$(function(){
	...
	var assetTab = $("#assetTab");	// 자산 탭 Element
	
	// 회의실 탭 클릭 시 이벤트 처리
	roomTab.on("click", function(){
		var tab = $(this).data("tab"); // 클릭한 탭의 정보를 
		$("#tab").val(tab); // tab 필드에 저장
		console.log(tab);
	});
	...
	// 자산 탭에서 2페이지 갔다가 회의실로 바로 가면 2페이지로 이동하는거 고치려고...
	$(function() {
	    // 탭 클릭 이벤트 핸들러 - 탭 간 전환 시 페이지 번호를 1로 재설정하고 해당 탭의 데이터를 로드
	    $('a[data-bs-toggle="tab"]').on('shown.bs.tab', function (e) {
	        // 탭 간 전환 시 페이지 번호를 1로 재설정
	        var selectedTab = $(e.target).data("tab");
	        $('input[name="page"]').val(1);
			//위 아래 코드 같이 써줘야 페이지가 리로드되면서 첫 번째 페이지로 감
	...
	roomPagingArea.on("click", "a", function(event){
		// 기본 동작(하이퍼링크 클릭에 의한 페이지 이동)을 막는 역할 - 페이지를 새로 고치는 대신 javascript코드로 페이지를 업데이트할 것을 의미
		event.preventDefault();
		// 클릭된 a태그에서 data-page 속성의 값을 읽어와서 pageNo 변수에 저장
	...
	assetPagingArea.on("click", "a", function(event){
		// 기본 동작(하이퍼링크 클릭에 의한 페이지 이동)을 막는 역할 - 페이지를 새로 고치는 대신 javascript코드로 페이지를 업데이트할 것을 의미
		event.preventDefault();
		// 클릭된 a태그에서 data-page 속성의 값을 읽어와서 pageNo변수에 저장 - 이 data-page 속성은 클릭된 페이지 번호를 나타내는 정보를 갖고 있어, 해당 페이지 번호를 가져옴
		var pageNo = $(this).data("page");
		// assetSearchForm라는 폼 내에서 id가 page인 input요소를 찾아서, 이 input 요소의 값을 클릭된 페이지 번호(pageNo)로 설정
...
//     e.preventDefault();
//     var selectedTab = $(this).attr('href'); // 선택한 탭의 ID 가져오기
//     console.log("선택한 탭: " + selectedTab);

//     // AJAX 요청을 통해 선택한 탭 정보를 서버로 전송
//     $.ajax({
//         type: "POST", // 요청 방식 (GET 또는 POST로 설정)
//         url: "/myrsv/list.do", // 컨트롤러 엔드포인트 URL로 변경해야 합니다.
//         dataType: 'json',
