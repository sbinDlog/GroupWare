...
<h2 class="uil-save"><a href="/draft/tempsave.do">기안함 </a> > 임시저장문서</h2>
<br/>
	<form action="/draft/tempsavedelete.do" method="post" id="delForm">
		<input type="hidden" name="atrzfCd" value="${draftVO.atrzfCd }" id="formCd">
	...
	<div>
		<table class="table table-sm" style="vertical-align: middle; height: 70px;">
		    ...
		    <tr>
		       ...
		       <td colspan="3" ><input class="form-control form-control-sm" type="text" id="drftTitle" name="drftTitle" value="${draftVO.drftTitle }"/></td>
		    </tr>
		    <tr>
		    ...
		    <c:choose>
		    	<c:when test="${not empty draftVO.drftStartdate }"> <!-- 날짜를 지정했으면  -->
				    <tr id="vacationDate">
				    	<th scope="col" style="width: 100px;">* 시작일</th>
						<td> 
							<c:set value="${draftVO.drftStartdate }"  var="stdate"/>
			        		<input class="form-control stDate" type="date" name="drftStartdate" style="width: 150px; margin-right: 10px;" value="${fn:substring(stdate, 0, 10)}"/>
			        	</td>
						<th style="width: 100px;">* 종료일</th>
						<td>
							<c:set value="${draftVO.drftEnddate }"  var="enddate"/>
							<input class="form-control endDate" type="date" name="drftEnddate" style="width: 150px; margin-left: 10px;" value="${fn:substring(enddate, 0, 10)}"/>
						</td>
				    </tr>
		    	</c:when>
		    	<c:otherwise>	
			    	 <tr id="vacationDate" style="display: none;"><!-- 날짜를 지정하지 않았으면  -->
				    	<th scope="col" style="width: 100px;">* 시작일</th>
						<td> 
		    ...
		    <tr>
		      ...
		      <td style="text-align: right;"><button class="btn btn-outline-info me-1 mb-1" type="button" id="treeButton">조직도</button></td>
		    </tr>
		</table>
	</div>
<br>
	<button class="float-end btn btn-outline-info me-1 mb-1" type="button" id="authLineButton">결재선 지정</button>
		<table class="table table-bordered" style="text-align: center;">
		    ...
		    <c:forEach items="${pathList}" var="path">
			    <tr>	
			    	<th scope="row">${path.atrzpOrder }</th>
			    	<td>승인</td>
			    	<td>${path.deptName}</td>
			    	<td>${path.empName } ${path.cdName }</td>
			    </tr>
				<input type="hidden" name="atrzpEmpno" value="${path.atrzpEmpno}_${path.atrzpOrder }" id="pathEmpno">
		    </c:forEach>
		  </tbody>
		</table>
	<br>
	...
	<div class="card-body">
		<div class="col-md-12">
			<textarea id="ckeditor" name="drftContent" class="form-control" rows="14">
				${draftVO.drftContent } </textarea>
		</div>
	</div>
<br>
	<div>
		<table class="table table-sm">
		    <tr>
		      ...
		      <td><textarea class="form-control" rows="5" cols="100" name="drftReason">${draftVO.drftReason }</textarea></td>
		      <td colspan="2"></td>
		    </tr>
		</table>
	</div>
	...
	<div style="text-align: center;">
		...
		<button class="uil-check btn btn-info me-1 mb-1" type="button" id="authBtn">결재 상신</button>
					    ...
					    <c:forEach items="${pathList }" var="path">
					    	<tr>
						      ...
						      <td>
						      	...
						      	<input type="hidden" name="atrzpEmpno" value="${path.empNo }">
						      </td>
						    </tr>
					    </c:forEach>
	...
	$('document').ready(function() {
		...
		okBtn.on("click", function(){
	        ...
	        if (authFormCd == "af001") {
	         ...
	         $("#delForm").append(`<input type="hidden" name="atrzfCd" value="`+authFormCd+`" id="formCd">`);
		...
		$("#tempBtn").on("click", function(){
			...
			if(drftTitle == ""){
				...
				delForm.attr("action", "/draft/tempsaveupdate.do");
				delForm.submit();	
			}	
		});
...
// 			var now = ($("input[name=atrzpEmpno]").length);
			var now = ($("#comfirmAuthList tr:gt(0)").length);
			console.log(now);
			
			if( $("#formCd").val() == "af001" && $("input[type=date]").val() == '') {
				alert("연차 날짜를 지정해주세요!");
			}else if (now !== 3) {
				...
				customConfirm("결재를 올리시겠습니까?").then((userConfirmed) => {
			        if (userConfirmed) { // yes버튼일떄
			       	 	delForm.attr("action", "/draft/tsapproval.do");
						delForm.submit();
			        } else { // no버튼일때
			            return;
			        }
		...
		listBtn.on("click", function(){
			location.href="/draft/tempsave.do";
		});
...
// 	if($("input[name=atrzpEmpno]").length > 2){ // 1) 결재선 4명인 상태에서 추가 선택시
// 		alert("더 이상 결재선을 추가할 수 없습니다.");
// 		return;
// 	}
...
//:gt(0)는 두 번째 tr행(인덱스 1)부터 그 이후 비우기
