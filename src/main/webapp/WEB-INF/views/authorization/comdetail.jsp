	...
	<div>
		<table class="table table-sm" style="vertical-align: middle; height: 70px;">
		    <tr>
		      ...
		      <td colspan="3" >${completeDetail.drftTitle }</td>
		    </tr>
		    <tr>
		      <th scope="row">* 기안자</th>
		      <td>${completeDetail.empName } ${completeDetail.cdName}/${completeDetail.deptName }</td>
		      <th>* 기안일</th>
		      <c:set var="drftDate" value="${completeDetail.drftDate }"/>
		      <td style=" width: 100px;">${fn:substring(drftDate,0,16) }</td>
		    </tr>
		    <tr> 
		    	<th scope="col">* 시작일</th> 
		    		<c:set var="stdate" value="${completeDetail.drftStartdate }"/>
				<td style="width: 150px; margin-right: 10px;">${fn:substring(stdate,0,10) }</td>
				<th style="width: 50px;">* 종료일</th> 
					<c:set var="enddate" value="${completeDetail.drftEnddate }"/>
				...
				<div>
					<table border="1" style="width: 450px; margin-right:150px; float: right; text-align: center; border-collapse: collapse; table-layout: fixed;">
						<tbody>
							...
							<tr>
								<!--결재자 이름  -->
								<td style="border: 1px solid black;">${completeDetail.empName }</td>
							...
							<tr style="height: 60px">
								<!--기안자 사인  -->
								<td style="border: 1px solid black;"><img width="50px;" src="${completeDetail.empSign }"></td> 
								<c:forEach items="${pathList }" var="path">
		...
		<table class="table table-sm">
		    <tr>
		      <th scope="col" style="width: 200px;">* 제출사유</th>
			      <c:choose>
				      	<c:when test="${empty completeDetail.drftReason }">
					      <td>미작성되었습니다.</td>
				      	</c:when>
						<c:otherwise>
							<td>${completeDetail.drftReason }</td>
						</c:otherwise>
			      </c:choose>
		      <td colspan="2"></td>
		    </tr>
