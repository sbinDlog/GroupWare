...
// 					str +='<hr/>';
					...
					if(item.fileApprse == 1 && userName != item.empNo) {
					} else {
					str +='<input type="hidden" value="'+item.fileCd+'" id="'+item.fileCd+'">';
					str +='<div class="row align-items-sm-center gx-2';
					...
					str += '</span>';
					if(userName == item.empNo && item.fileApprse == 1) {
						str += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button class="uil-ban btn btn-outline-danger resCancleBtn" type="button" data-value="'+item.fileCd+'" data-name="'+item.empNo+'">요청 삭제</button>';
					}
					str += '</div>';
