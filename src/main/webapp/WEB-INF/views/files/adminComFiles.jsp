...
<h2>전사자료실</h2>
<span style="font-weight: bolder;">I-WORKS의<span style="color: blue;"> 전체 자료실</span>입니다.</span>
		...
		<div class="d-flex justify-content-center" >
			...
			<div class="col-6 px-7">
				<div class="d-felx text-start">
					<button class="uil-folder-open btn btn-outline-warning" type="button" id="apprBtn">업로드요청목록</button>
				</div>
			</div>
			<div class="col-6">
				<div class="d-felx text-end mb-3">
...
<%-- 		</c:if> --%>
			...
			</c:if>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;현재 경로&nbsp;:&nbsp;전사자료실${parentCd.folderOwnPath }</h5>
<%-- 			<c:if test="${not empty back }"> --%>
<!-- 			<div class="col-2"> -->
			
<!-- 				<span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span> -->
<!-- 				<button class="uil-arrow-circle-left btn btn-link me-1 mb-1" type="button" id="backBtn">이전</button> -->
<!-- 				<button class="btn btn-link me-1 mb-1" type="button" id="atStartBtn">처음으로</button> -->
...
<%-- 			</c:if> --%>
		...
		<div style="display: flex;">
			...
			<div style="display: flex;" class="ms-12">
				<form action="/comSelectChildFileBasicAdmin.do" method="POST" id="folderFileDateBasic">
				...
				<form action="/comSelectChildFileDateAdmin.do" method="POST" id="folderFileDateForm">
...
<!-- 				<button class="btn btn-link me-1 mb-1" type="button" id="basicSort">기본순</button> -->
<%-- 				<input type="hidden" value="${parentCd.folderCd }" name="folderParent"> --%>
<!-- 			</form> -->
<!-- 		</div> -->
<!-- 		<div class="col-1 text-end"> -->
<!-- 			<form action="/comSelectChildFileDate.do" method="POST" id="folderFileDateForm"> -->
<!-- 				<button class="btn btn-link me-1 mb-1" type="button" id="DateSort">최신순</button> -->
<%-- 				<input type="hidden" value="${parentCd.folderCd }" name="folderParent"> --%>
<!-- 			</form> -->
<!-- 		</div> -->
	<div class="row mb-8">
...
<!-- 				</div> -->
				...
				<div class="border-top border-200 hover-actions-trigger py-3" id="folderFileDiv">
					...
					<form action="/adminComfiles.do" method="POST" id="${folder.folderName }">
...
// 					str +='<hr/>';
					...
					str += '<input type="hidden" name="fileCd" value="'+item.fileCd+'" id="'+item.fileCd+'">';
					str += '<div class="row align-items-sm-center gx-2 ';
					if(item.fileApprse == 1) {
						str += 'bg-light';
					}
					str += '">';
					str += '<div class="col-auto">';
					...
					str += '</div>';
					str += '<div class="col-6">';
					str += '<span class="text-1100 inbox-link fileSelect" data-value="'+item.fileCd+'" style="cursor:pointer">'+item.fileOrgname+'.'+item.fileType;
					if(item.fileApprse == 1) {
						str += '<span class="text-danger fw-bold">&nbsp;&nbsp;(승인대기중)</span>';
					}
					str += '</span>';
					if(item.fileApprse == 1) {
						str += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button class="uil-circle btn btn-outline-info resYes" type="button" data-value="'+item.fileCd+'" data-name="'+item.empNo+'">승인</button>';
						str += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button class="uil-ban btn btn-outline-danger resNo" type="button" data-value="'+item.fileCd+'" data-name="'+item.empNo+'">반려</button>';
					}
					str += '</div>';
					str += '<div class="col-2 text-end">';
					str += '<span>'+item.fileSize+'KB</span>';
					str += '</div>';
					
					var formatDate = new Date(item.fileUploaddt);
					var year = formatDate.getFullYear();
					var month = (formatDate.getMonth() +1).toString().padStart(2, '0');
					var day = formatDate.getDate().toString().padStart(2, '0');
					var hour = formatDate.getHours().toString().padStart(2, '0');
					var minute = formatDate.getMinutes().toString().padStart(2, '0');
					
					str += '<div class="col-3 text-end">';
					str += '<span>'+year+'.'+month+'.'+day+'.'+' '+hour+':'+minute+'</span>';
					str += '</div>';
					str += '</div>';
					str += '<hr/>';
					str += '';
...
// 	deleteFileBtn.on('click', function(){
	...
	$(document).on('click', '#apprBtn', function(){
		location.href="/comFileApprList.do";
	});
});
</script>
