...
<h2>개인자료실</h2>
<span style="font-weight: bolder;">${emp.empName }님의 <span style="color: blue;">개인 자료실</span>입니다.</span>
...
<!-- 		<div class="col-8"></div> -->
			<div class="d-felx text-end">
				...
				<button class="uil-folder-plus btn btn-info me-1 mb-1" type="button" id="resFolderBtn">폴더생성</button>
				<button class="uil-file-upload btn btn-outline-info me-1 mb-1" type="button" id="resFileBtn">파일업로드</button>
			</div>
<!-- 		</div> -->
<!-- 	</div> -->
	<div class="row">
			<h5 class="pt-3 text-500"><c:if test="${empty back }">
			</c:if>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;현재 경로&nbsp;:&nbsp;개인자료실${parentCd.folderOwnPath }</h5>
		</div>
		
		<div style="display: flex;">
			<c:if test="${not empty back }">
				<div class="col-9">
					<button class="uil-arrow-circle-left btn btn-link ms-2 fs-0" type="button" id="backBtn">이전으로</button>
			...
			<div style="display: flex;" class="ms-12">
				<form action="/selectChildFileBasic.do" method="POST" id="folderFileDateBasic">
				...
				<form action="/selectChildFileDate.do" method="POST" id="folderFileDateForm">
...
<div class="modal fade" id="showPictureModal" tabindex="-1" aria-labelledby="verticallyCenteredModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="verticallyCenteredModalLabel">이미지 미리보기</h5>
...
<div class="modal fade" id="showFileModal" tabindex="-1" aria-labelledby="verticallyCenteredModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="verticallyCenteredModalLabel">파일 미리보기</h5>
        <button class="btn p-1" type="button" data-bs-dismiss="modal" aria-label="Close">
        <span class="fas fa-times fs--1"></span></button>
      </div>
      <div class="modal-body d-flex justify-content-center align-items-center">
...
<form action="indSelectBack.do" method="post" id="backForm">
	<input type="hidden" value="${parentCd.folderCd }" name="folderParent">
</form>
...
// 					str +='<form action="/indfiles.do" method="POST" id="'+item.folderName+'">';
...
// 					str +='<hr/>';
					...
					str +=' data-bulk-select-row="data-bulk-select-row"/>';
					str +='<div id="">';
					str += '<button class="btn p-0 fileLike" type="button" style="transform: scale(1);" data-value="'+item.fileCd+'" style="cursor:pointer">';
					if(item.fileLikese == 2){
						str += '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-star-fill text-warning" viewBox="0 0 16 16">';
						...
						str += '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-star text-warning" viewBox="0 0 16 16">';
						str += '<path d="M2.866 14.85c-.078.444.36.791.746.593l4.39-2.256 4.389 2.256c.386.198.824-.149.746-.592l-.83-4.73 3.522-3.356c.33-.314.16-.888-.282-.95l-4.898-.696L8.465.792a.513.513 0 0 0-.927 0L5.354 5.12l-4.898.696c-.441.062-.612.636-.283.95l3.523 3.356-.83 4.73zm4.905-2.767-3.686 1.894.694-3.957a.565.565 0 0 0-.163-.505L1.71 6.745l4.052-.576a.525.525 0 0 0 .393-.288L8 2.223l1.847 3.658a.525.525 0 0 0 .393.288l4.052.575-2.906 2.77a.565.565 0 0 0-.163.506l.694 3.957-3.686-1.894a.503.503 0 0 0-.461 0z"/>';
						str += '</svg>';
					}
					str += '</button>';
					str += '</div>';
	...
	$(document).on("click",".fileLike",function(){
