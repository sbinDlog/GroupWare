...
<%-- 		</c:if> --%>
	...
	<div style="display: flex;">
		...
		<div style="display: flex;" class="ms-12">
			<form action="/depSelectChildFileBasic.do" method="POST" id="folderFileDateBasic">
			...
			<form action="/depSelectChildFileDate.do" method="POST" id="folderFileDateForm">
...
<!-- 				</div> -->
				...
				<div class="border-top border-200 hover-actions-trigger py-3" id="folderFileDiv">
					...
					<form action="/depfiles.do" method="POST" id="${folder.folderName }">
...
<!-- 폴더이름을 저장할 모달. -->
<form action="/insertDepFileFolder.do" method="POST" id="folderNameForm">
...
<!-- 파일 넘기기 -->
<form action="/insertDepFileFile.do" method="POST" id="fileNameForm" enctype="multipart/form-data">
...
<!-- 체크박스 선택된 애들의 삭제 폼 -->
<form action="depFileDelete.do" method="POST" id="selectBoxDelForm">
	<input type="hidden" value="" name="folderArray" class="folderSelectArray"/>
	<input type="hidden" value="" name="fileArray" class="fileSelectArray"/>
...
<form action="/depSelectBack.do" method="post" id="backForm">
	<input type="hidden" value="${parentCd.folderCd }" name="folderParent">
</form>
...
//		 		fileSelctBox.each(function () {
				...
				selectBoxForm.attr('action','/depShare.do')
