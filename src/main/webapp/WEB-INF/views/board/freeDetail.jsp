			...
			<div>
				<div class="container-fluid">
					<div class="row d-flex justify-content-center text-center" style="padding:20px;">
						<h3 id="title">${free.frTitle}</h3>
					</div>
					<div class="text-end" id="likeDiv">
						<c:choose>
						<c:when test="${like gt 0}">
							<button id="likeDownBtn" class="btn p-0" data-frno="${free.frNo}"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-heart likedown" style="height: 30px; width: 30px; color: red;"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg></button>
						</c:when>
						<c:otherwise>
							<button id="likeUpBtn" class="btn p-0" data-frno="${free.frNo}"><span class="likeup"  data-feather ="heart" style=" height: 30px; width: 30px; color: red;"></span></button>
						</c:otherwise>
						</c:choose>
					</div>
				</div>
				...
				<div class="d-flex justify-content-between align-items-center border-0" style="padding:30px;">
						...
						<c:choose>
						    <c:when test="${free.frAnonySe eq 'Y'}">
						        <div><span>익명</span> / <span>${freeDate}</span></div>
						    </c:when>
						    <c:otherwise>
						        <div><span>${free.empName}</span> / <span>${freeDate}</span></div>
						    </c:otherwise>
					  </c:choose>
					  <input type="hidden" value="${free.frWriterEmpNo}" id="empNo">
					  <div>조회수 : ${free.frHit}<span></span></div>
					</div>
				</div>
...
<!-- 						    <button class="btn btn-info me-1 mb-1" type="button" id="deleteBtn">삭제</button> -->
<!-- 						    <button class="btn btn-outline-info me-1 mb-1" type="button" id="listBtn">목록</button> -->
								<button class="btn btn-phoenix-secondary" id="modifyBtn">
							    ...
							    <button class="btn btn-phoenix-danger" id="deleteBtn">
...
<!-- 						  	<button class="btn btn-outline-info me-1 mb-1" type="button" id="listBtn">목록</button> -->
					...
					<div class="row">
						...
						<form id="quickForm" novalidate="novalidate">
							<div class="card-body" style="padding:30px;">${free.frContent}</div>
								<div>
...
// 			$("#comment").html("");
	...
	$("#comment").on("click", "button[name='delCommentBtn']", function(){
		...
		customConfirm('정말 삭제하시겠습니까?').then((userConfirmed) => {
          if (userConfirmed) { 
        	  ...
        	  $.ajax({
      			type: "get",
      			url: "/free/comment/remove.do?cmNo="+cmNo,
      			success: function(result){
      				if(result == 'SUCCESS'){
	...
	$(document).on("click", "#likeUpBtn", function () {
	    var likeBtn = $(this);
	    var frNo = likeBtn.data("frno");
	    console.log("likeUpBtn clicked");

	    $.ajax({
	        type: "POST",
	        url: "/free/like/insert.do",
	        data: {
	            frNo: frNo
	        },
	        success: function (response) {
	            likeBtn.replaceWith('<button id="likeDownBtn" class="btn p-0" data-frno="' + frNo + '"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-heart likedown" style="height: 30px; width: 30px; color: red;"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"></path></svg></button>');
	            feather.replace(); 
	        }
	    });
	});

	$(document).on("click", "#likeDownBtn", function () {
	    var likeBtn = $(this);
	    var frNo = likeBtn.data("frno");
	    console.log("likeDownBtn clicked");

	    $.ajax({
	        type: "GET",
	        url: "/free/like/remove.do",
	        data: {
	            frNo: frNo
	        },
	        success: function (response) {
	            likeBtn.replaceWith('<button id="likeUpBtn" class="btn p-0" data-frno="' + frNo + '"><span class="likeup" data-feather ="heart"style="height: 30px; width: 30px; color: red;"></span></button>');
	            feather.replace(); 
	        }
	    });
	});
